	...
	<main>
		...
		<div class="main-con">
									...
									<tbody>
										<tr>
											...
											<td>
												...
												<select name="res_doctor" id="res_doctor" style="display: inline-block;">
											...
											<td>
												<span class="mFormRequired">
												<select name="res_status" id="res_status" style="display: inline-block;">
													<option value="">선택</option>
	    	...
	    	<!-- 버튼 공통 -->
	     		...
	     		<div class="input-btn0">	
					<input type="button" value="전체 목록으로 돌아가기" onclick="javascript:location.href='adcheckRes.do';"/>
				</div>	
    		</form>	    		    			
		</div> <!-- main-con:e -->
    	...
    	<div class="adpage">
    		<c:if test="${ currentPage <= 1 }">
    			...
    			<c:url var="ubl" value="resByStatus.do">
					<c:param name="page" value="1" />
					<c:param name="res_status" value="${ requestScope.res_status }" />
				</c:url>
			...
			<c:forEach var="p" begin="${ startPage }" end="${ endPage }" step="1">
				<c:if test="${ p eq currentPage }">
					...
					<c:url var="ubl2" value="resByStatus.do">
    		...
    		<c:if test="${ currentPage >= maxPage }">
				...
				<c:url var="ubl3" value="resByStatus.do">
