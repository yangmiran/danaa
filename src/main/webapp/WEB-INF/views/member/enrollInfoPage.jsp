...
<!-- 회원가입STEP03 회원정보 입력 페이지  (일반 회원가입)-->
...
//form태그에서 가입하기/본인인증 버튼에 따라 action 값 다르게 설정
	...
	//	$('#birth_check').text('');
		...
		if(idchk ==0){
			...
			$('#id_check').text('아이디 중복확인 체크를 해주세요');
			$('#id_check').css('color', 'red');
					
		}
		
		//패스워드 값과 확인 값 다름
		 if($('#pwd').val() !== $('#pwdck').val() || $('#pwd').val()==''||$('#pwd_check').val()!=''){
			//alert('입력 정보를 확인해주세요');
			$('#pwd_check').text('패스워드를 확인해주세요');
			$('#pwd_check').css('color', 'red');			
			
		} 
		
		//생년월일 체크
		if($('#birth_check').val()!=='' || $('#birth').val()==''){
			//alert('입력 정보를 확인해주세요');
		...
		//이메일 중복확인 체크
		if(emailchk==0){
			$('#email_check').text('아이디 중복확인 체크를 해주세요');
		...
		if(birthdatechk == 'N'&& (yearNow-$('#birth').val().substr(0, 4))>=14){ //14세 이상+체크 제대로함
		 ...
		 if(idchk ==0 || ($('#pwd').val() !== $('#pwdck').val() || $('#pwd').val()==''||$('#pwd_check').val()!='') || ($('#birth_check').val()!=='' || $('#birth').val()=='') || ($('#name_check').val()!='' || $('#name').val()=='') || (emailchk==0) ||( (birthdatechk == 'N'&& (yearNow-$('#birth').val().substr(0, 4))>=14) && ($('#phone_check').text()!='인증 완료'|| ($('#phonekey').val() != key && phonesend==1)) )|| ( !(birthdatechk == 'N'&& (yearNow-$('#birth').val().substr(0, 4))>=14) && !(birthdatechk == 'Y'&& (yearNow-$('#birth').val().substr(0, 4))<14) )){
			alert('입력 정보를 확인해주세요');
			return false;
		}  
...
<%-- 	 function enrollSubmit2(frm) {//본인인증
		 if (phoneJ.test($('#phone').val())) {//유효성 검사 통과시
			 frm.action = 'test2.do'; //**action 값 sendsms.do로 바꿔서 sms전송이 되게 해야함**
				frm.submit();
			 	alert("인증번호 전송이 완료되었습니다");
				return true;
		} else { //유효성 검사 통과 못함
			 $('#phone_check').text('휴대폰  형식이 맞지 않습니다');//휴대폰 유효성 불일치시
			 $('#phone_check').css('color', 'red');
	...
	//인증번호 확인
	 function checkKey() {
		...
		var key ='<%=(String)session.getAttribute("key")%>';
			if($('#phonekey').val() !== key){
				$('#phone_check').text('인증번호가 일치하지 않습니다');//인증번호와 불일치시
				$('#phone_check').css('color', 'red');
					
			}else{
				$('#phone_check').text('확인 완료');//인증번호와 일치시
				$('#phone_check').css('color', 'blue');
			}
	} --%>	
	...
	//아이디 중복 체크 확인	 
	function dupIdCheck() {
		if (idJ.test($('#id').val()) && idchk==0) { //아이디 유효성 검사 통과시 중복체크함
			$.ajax({
				url : "/danaa/idcheck.do",
				type : "post",
				data : {
					id : $("#id").val()
				},
				success : function(result) {
					if (result != 1) {
						alert("사용 가능한 아이디입니다.");
						$("#pwd").focus();
						$('#pwd').focus(function() { //포커스가 pwd로 처음가면 멘트 리셋
							$('#pwd_check').text('');	
						});
						$('#id_check').text('');
			...
			return false;
		}else if(idJ.test($('#id').val()) && idchk==1){ //중복 체크를 완료 했다면
			alert('이미 중복 체크를 완료하였습니다.')
		}else {//아이디 유효성 검사에 적합하지 않을 경우 
			$('#id_check').text('아이디를 확인해주세요');
			$('#id_check').css('color', 'red');
			$('#id').focus(function() { //포커스가 id로 가면  멘트 리셋
				$('#id_check').text('');	
			});
		}
	...
	<div class="enroll_info">
		<div class="tableWrap">
			...
			<form:form method="post" id="allsendForm" name="allsendForm"
				action="menroll.do" onsubmit='return enrollSubmit1()' modelAttribute="member">
				...
				<table>
					...
					<tbody>
						<%-- <tr>
							<th><label for="under14">14세 미만여부</label><!-- <span class="requiredTxt">*</span> --></th>
							<td>
							<form:checkbox path="child" value="Y" item="${child}"/>
							<div class="check_font" id="under14_check"></div> 
							</td>
						</tr>  --%>
						...
						<tr>
							...
							<td>
								<form:input path="id" id="id" name="id" class="inputTxt" type="text"
								maxlength="20"/>
								<input type="button" class="id_ch"
								onclick='dupIdCheck();' style="opacity: 1; cursor:pointer;"
								onmouseover="this.style.opacity='0.7'"
								onmouseout="this.style.opacity='1'" value="중복확인"> 
								 <div class="check_font" id="id_check"></div> 
								<div class="spring_error">
								<font color="red">
								<spring:hasBindErrors name="member"> 
           							<c:if test="${errors.hasFieldErrors('id') }">                                    
               		 					<form:errors path="id" id="id_check2"/>               	
               						 </c:if> 
								</spring:hasBindErrors>
								</font> 
								</div>
								<span class="id_txt">영문자, 숫자만 입력 가능. 최소 4자이상 입력하세요.</span>
						...
						<tr>
							...
							<td class="mail_type">								
								...
								style="opacity: 1; cursor:pointer;" onmouseover="this.style.opacity='0.7'"
						...
						<tr>
							<th>휴대폰 번호<br><!-- <p>(14세 미만은 필수 아님)</p> --></th>
							
							<td>
							<form:input path="phone" id="phone" name="phone" class="inputPhone" type="text" maxlength="20"/>
							
								
								<input type='button' onclick='return enrollSubmit2();' value='인증번호 전송' style="opacity: 1; cursor:pointer;" onmouseover="this.style.opacity='0.7'"
								onmouseout="this.style.opacity='1'"> 
								<div id="phone_chk_com" class="phone_chk_com">
								<input type="text" name="phonekey" id="phonekey" class="phonekey" maxlength="13"> 			
								<input type='button'onclick='checkKey();' value='인증번호 확인' style="opacity: 1; cursor:pointer;" onmouseover="this.style.opacity='0.7'"
								onmouseout="this.style.opacity='1'"> 															
								</div>
				...
				<!-- <div class="enroll_button btn_common_n"> -->
				<!-- <input type='button' value='이전단계 ' style="opacity: 1; cursor:pointer;" onmouseover="this.style.opacity='0.7'"
						onmouseout="this.style.opacity='1'" class="enroll_button btn_common_n1" onclick='history.go(-1); return false;'> -->
